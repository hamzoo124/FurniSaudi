...
const AddProduct: React.FC = () => {
  ...
  const getCategorizedProductTypes = () => {
    const result = {
      ready_made: {
        indoor: [] as any[],
        outdoor: [] as any[],
    ...
    const categorizedTypes = {
      ready_made_indoor: new Set<string>(),
      ready_made_outdoor: new Set<string>(),
      ready_made_both: new Set<string>(),
      customized_indoor: new Set<string>(),
      customized_outdoor: new Set<string>(),
      customized_both: new Set<string>()
    };

    categories.forEach(cat => {
      if (cat.product_type && cat.is_active) {
        allProductTypes.add(cat.product_type);
        
        if (cat.category_type === 'ready_made') {
          if (cat.usage_type === 'indoor') categorizedTypes.ready_made_indoor.add(cat.product_type);
          if (cat.usage_type === 'outdoor') categorizedTypes.ready_made_outdoor.add(cat.product_type);
          if (cat.usage_type === 'both') categorizedTypes.ready_made_both.add(cat.product_type);
        } else if (cat.category_type === 'customized') {
          if (cat.usage_type === 'indoor') categorizedTypes.customized_indoor.add(cat.product_type);
          if (cat.usage_type === 'outdoor') categorizedTypes.customized_outdoor.add(cat.product_type);
          if (cat.usage_type === 'both') categorizedTypes.customized_both.add(cat.product_type);
        }
      }
    });

    const productTypeArray = Array.from(allProductTypes).map(type => ({
      value: type,
      label: type.charAt(0).toUpperCase() + type.slice(1).replace(/_/g, ' ')
    }));

    productTypeArray.forEach(type => {
      if (categorizedTypes.ready_made_indoor.has(type.value)) {
        result.ready_made.indoor.push(type);
      }
      if (categorizedTypes.ready_made_outdoor.has(type.value)) {
        result.ready_made.outdoor.push(type);
      }
      if (categorizedTypes.ready_made_both.has(type.value)) {
        result.ready_made.both.push(type);
      }
      if (categorizedTypes.customized_indoor.has(type.value)) {
        result.customized.indoor.push(type);
      }
      if (categorizedTypes.customized_outdoor.has(type.value)) {
        result.customized.outdoor.push(type);
      }
      if (categorizedTypes.customized_both.has(type.value)) {
        result.customized.both.push(type);
      }
    });
  ...
  const categorizedProductTypes = getCategorizedProductTypes();
    ...
    if (selectedCategory) {
      ...
      if (selectedCat) {
        setFormData(prev => ({
          ...
          usage_type: selectedCat.usage_type,
          product_type: selectedCat.product_type || '',
          product_categories: selectedCat.product_categories || []
        }));
      }
    }
  ...
  const getAvailableProductTypes = () => {
    const types = categorizedProductTypes[formData.category_type]?.[formData.usage_type] || [];
    if (types.length === 0) {
      const allTypes = new Set<string>();
      categories.forEach(cat => {
        if (cat.product_type && cat.is_active) {
          allTypes.add(cat.product_type);
        }
      });
  ...
  const handleProductCategoryChange = (category: string) => {
    setFormData(prev => {
      ...
      return { ...prev, product_categories: newCategories, category_id: '' };
    });
    setSelectedCategory('');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productToSave = {
        ...
        is_featured: false,
        seller_id: user?.id || 'seller_001',
        seller_name: user?.name || user?.email || 'Sample Seller',
        images: formData.images.map(id => getImageUrl(id)).filter(url => url),
        main_image: formData.mainImage ? getImageUrl(formData.mainImage) : null,
        ...
        warranty_type: formData.warrantyType || null,
        warranty_duration: formData.warrantyDuration || null,
        has_variants: formData.hasVariants,
        variants: formData.variants,
        shipping_info: {
          available_cities: formData.availableCities,
          shipping_options: formData.shippingOptions
        },
        installation_available: formData.installationAvailable,
        installation_fee: formData.installationFee || '0',
        return_policy: formData.returnPolicy,
        customization_time: formData.customizationTime || null,
        materials_used: formData.materialsUsed || [],
        display_section: formData.displaySection || null,
        rating: 0,
        orders: 0,
        reviews: [],
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Select Admin Category (Optional)
                  </label>
                  <select
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Selecting a category will auto-fill category fields below
                  </p>
                </div>
              </div>
              ...
              {formData.category_type && formData.usage_type && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Product Type *
                  </label>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Product types are filtered based on your Category Type and Usage Type selection
                  </p>
                </div>
              )}
                  ...
                  <div className={`border ${errors.product_categories ? 'border-red-500' : 'border-gray-300'} rounded-lg p-4`}>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {productCategoryOptions.map(category => (
                        <label key={category.value} className="flex items-center space-x-3 cursor-pointer p-3 hover:bg-gray-50 rounded-lg border border-gray-200">
                          <input
                            type="checkbox"
                            checked={formData.product_categories.includes(category.value)}
                            onChange={() => handleProductCategoryChange(category.value)}
                  ...
                  {formData.product_categories.length > 0 && (
                    <p className="text-sm text-gray-600 mt-3">
                      ...
                      {formData.product_categories.map(cat => {
                        const catOption = productCategoryOptions.find(c => c.value === cat);
                        return catOption ? catOption.label : cat;
                      }).join(', ')}
                    </p>
                  )}
                </div>
